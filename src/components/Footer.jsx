    ...
    const sections = [
        {
            ...
            links: ['iPhone 17Pro', 'iPhone 17Pro Max', 'Comprar modelos', 'Acessórios']
        },
        {
            title: 'Especificamente',
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                   ...
                   <div className="flex gap-6 text-sm text-gray-400">
                     {buttonLinks.map((button, index) => (
                        <a href="#" key={index} className="hover:text-white">{button}</a>
                    ))}
                   </div>
                </div>
                <p className="text-xs text-gray-500 mt-6 text-center">Site criado para fins educacionais</p>
            </div>
            </div>
        </footer>
    )
